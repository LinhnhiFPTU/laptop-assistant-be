...
class RequestQueue {
  ...
  async execute(fn) {
    ...
    if (timeSinceLastRequest < this.minDelayMs) {
      ...
      console.log(`Throttling protection: waiting ${delayNeeded}ms before next request`);
...
class SupervisorAgent {
    ...
    for (let i = 0; i <= retries; i++) {
      try {
        ...
        return await bedrockQueue.execute(async () => {
          console.log(`Sending Bedrock request (attempt ${i+1}/${retries+1})`);
          const res = await bedrock.send(new InvokeModelCommand(payload));
          return JSON.parse(await res.body.transformToString());
        });
      } catch (err) {
        if (err.name === "ThrottlingException" && i < retries) {
          console.log(`ThrottlingException received, waiting ${delay}ms before retry`);
  ...
  static async aggregateResponses(question, agentResults) {
      ...
      messages: [
        {
          ...
          ${agentResults.map(r => `- ${r.agentName}: ${r.data || "Không có thông tin"}`).join("\n")}
    ...
    try {
      ...
      if (error.name === "ThrottlingException") {
        ...
        console.log("Even fallback model is throttled, generating simple response");
        ...
        if (simpleResponse === `Tôi đã tìm được thông tin sau:\n\n`) {
          simpleResponse = "Xin lỗi, hiện tại hệ thống đang quá tải. Vui lòng thử lại sau ít phút.";
        }
  ...
  static async processQuery(question, token) {
    try {
      ...
      if (analysis.needsVectorSearch) {
        promises.push(
          VectorAgent.searchKB(question)
            .then(data => agentResults.push({ agentName: "VectorAgent", data }))
            .catch(error => {
        ...
        if (token) {
          promises.push(
            OrderAgent.getOrderContext(token)
              .then(data => agentResults.push({ agentName: "OrderAgent", data }))
              .catch(error => {
      ...
      if (analysis.needsPromotionInfo) {
        promises.push(
          PromotionAgent.getPromotionContext()
            .then(data => agentResults.push({ agentName: "PromotionAgent", data }))
            .catch(error => {
      ...
      const allFailed = agentResults.length > 0 && 
